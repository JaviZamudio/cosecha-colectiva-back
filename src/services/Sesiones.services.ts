...
export const actualizar_intereses = async (Grupo_id: number) => {
    ...
    const query2 = "UPDATE prestamos INNER JOIN sesiones ON prestamos.Sesion_id = sesiones.Sesion_id INNER JOIN socios ON prestamos.Socio_id = socios.Socio_id INNER JOIN acuerdos ON prestamos.Acuerdos_id = Acuerdos.Acuerdo_id SET prestamos.Interes_generado = prestamos.Interes_generado + ((prestamos.Monto_prestamo*acuerdos.Interes_morosidad)/100 + (prestamos.Monto_prestamo*acuerdos.Tasa_interes)/100) WHERE sesiones.Grupo_id = ? AND prestamos.Estatus_prestamo = 0 AND prestamos.Estatus_ampliacion = 0 AND prestamos.Sesiones_restantes < 0 AND prestamos.Prestamo_original_id IS NULL AND socios.`Status` = 1;";
    await db.query(query2, [Grupo_id]);
    //intereses ampliacion --- Interes_ampliacion --- %
    const query3 = "UPDATE prestamos INNER JOIN sesiones ON prestamos.Sesion_id = sesiones.Sesion_id INNER JOIN socios ON prestamos.Socio_id = socios.Socio_id INNER JOIN acuerdos ON prestamos.Acuerdos_id = Acuerdos.Acuerdo_id SET prestamos.Interes_generado = prestamos.Interes_generado + ((prestamos.Monto_prestamo*acuerdos.Interes_ampliacion)/100 + (prestamos.Monto_prestamo*acuerdos.Tasa_interes)/100) WHERE sesiones.Grupo_id = ? AND prestamos.Estatus_prestamo = 0 AND prestamos.Estatus_ampliacion = 1 AND prestamos.Sesiones_restantes >= 0 AND prestamos.Prestamo_original_id IS NOT NULL AND socios.`Status` = 1;";
...
export const agregar_interes_prestamo = async (Grupo_id: number) => {
    ...
    try{
        ...
        //intereses morosidad --- Interes_morosidad --- %
        const query2 = "SELECT Prestamo_id, prestamos.Interes_generado + ((prestamos.Monto_prestamo*acuerdos.Interes_morosidad)/100 + (prestamos.Monto_prestamo*acuerdos.Tasa_interes)/100) as interesGenerado FROM prestamos INNER JOIN sesiones ON prestamos.Sesion_id = sesiones.Sesion_id INNER JOIN socios ON prestamos.Socio_id = socios.Socio_id INNER JOIN acuerdos ON prestamos.Acuerdos_id = Acuerdos.Acuerdo_id WHERE sesiones.Grupo_id = ? AND prestamos.Estatus_prestamo = 0 AND prestamos.Estatus_ampliacion = 0 AND prestamos.Sesiones_restantes < 0 AND prestamos.Prestamo_original_id IS NULL AND socios.`Status` = 1;";
        ...
        //intereses ampliacion --- Interes_ampliacion --- %
        const query3 = "SELECT Prestamo_id, prestamos.Interes_generado + ((prestamos.Monto_prestamo*acuerdos.Interes_ampliacion)/100 + (prestamos.Monto_prestamo*acuerdos.Tasa_interes)/100) as interesGenerado FROM prestamos INNER JOIN sesiones ON prestamos.Sesion_id = sesiones.Sesion_id INNER JOIN socios ON prestamos.Socio_id = socios.Socio_id INNER JOIN acuerdos ON prestamos.Acuerdos_id = Acuerdos.Acuerdo_id WHERE sesiones.Grupo_id = ? AND prestamos.Estatus_prestamo = 0 AND prestamos.Estatus_ampliacion = 1 AND prestamos.Sesiones_restantes >= 0 AND prestamos.Prestamo_original_id IS NOT NULL AND socios.`Status` = 1;";
