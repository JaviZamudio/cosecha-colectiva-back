...
// resBody = { Prestamo_id: number, Adeudo_prestamo: number, Adeudo_interes: number, Sesiones_restantes: number }
...
export const get_historial_pres_socio_ses = async (req: AdminRequest<any>, res) => {
    ...
    try {
        let prestamosQuery = "SELECT Transaccion_id,Timestamp,Cantidad_movimiento,Catalogo_id FROM transacciones WHERE (Sesion_id = ? AND Socio_id = ?) AND (Catalogo_id = 'ABONO_PRESTAMO' OR Catalogo_id = 'ENTREGA_PRESTAMO')";
        const [prestamos] = await db.query(prestamosQuery, [Sesion_id, Socio_id]);
