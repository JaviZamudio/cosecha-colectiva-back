...
export const get_usuario_status = async (req: AdminRequest<Grupo>, res) => {
    ...
    try {
        ...
        let query = "SELECT so.Socio_id, CONCAT(so.Nombres, ' ', so.Apellidos) AS Nombre, gs.Status FROM socios so JOIN grupo_socio gs ON so.Socio_id = gs.Socio_id WHERE gs.Grupo_id = ? AND gs.Status = 1 AND gs.Status = 0;";
